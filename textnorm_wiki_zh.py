...
## regrex patterns
pat_docstart=re.compile(u'^<doc\s+id=\"(.+)\"\s+url=\"(.+)\"\s+title=\"(.+)\">')

pat_comments=re.compile(u'[\(\uff08][^\(\uff08\)\uff09]*[\)\uff09]')

pat_year=re.compile(u'([0-9]{4}\u5e74)') # .{1,4}year
...
pat_nonchar=re.compile(u'[^%s]' % zhon.unicode.HAN_IDEOGRAPHS)
...
def splitPar(sr):
	...
	for itm in lst:
		...
		stlst=[]
		if len(st)>50:
			stlst=pat_comma.split(st)
			if debug:
				print u'###############'
				for st in stlst: print st
		else:
			stlst.append(st)
			if debug:
				print u'=============='
		...
		for st in stlst:
			st=pat_punc.sub(u'', st)
			st=pat_space.sub(u'', st)
			st=pat_special1.sub(u'\u81f3', st)
			if pat_nonchar.search(st): # ignore sentences with non Chinese characters
...
fin=open('texts/wiki_00_chs', 'rb')
fout=open('texts/wiki_00_chs_sents', 'w')
...
while True:
	...
	if sr==u'':break
	sr=sr.strip()
	if debug and sr==u'</doc>': sys.exit(1)
	if pat_docstart.match(sr) or sr==u'</doc>':
		continue
	else:
		if sr!=u'':
			# remove the English comments in the form of "(abc)"
			par=replaceComments(sr)
